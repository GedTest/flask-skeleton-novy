...
from .forms import LogUserForm, secti, masoform, ocForm, ValidateStock
...
def scitani():
    form = secti()
    if form.validate_on_submit():
        return render_template('public/vystup.tmpl', hod1=form.hodnota1.data, hod2=form.hodnota2.data,
                               suma=form.hodnota1.data + form.hodnota2.data)
...
def masof():
    form = masoform()
    if form.validate_on_submit():
        return render_template('public/masovystup.tmpl', hod1=form.hodnota1.data, hod2=form.hodnota2.data,
                               suma=form.hodnota1.data + form.hodnota2.data)
...
# ----------------------------------------------------------------------------------------------------------------------
@blueprint.route("/StockAdd")
def StockAdd():
    data = Stock(firma="ViktorZwinger", cena=100)
    db.session.add(data)
    db.session.commit()

    return "OK"
# ----------------------------------------------------------------------------------------------------------------------
@blueprint.route("/StockAddForm", methods=['GET', 'POST'])
def StockAddForm():
    form = ValidateStock()
    if form.validate_on_submit():
        Stock.create(form.data)
    return render_template('public/stockform.tmpl', form=form)
# ----------------------------------------------------------------------------------------------------------------------
@blueprint.route('/StockList')
def StockList():
    list = db.session.query(Stock).all()
    return render_template('public/StockList.tmpl', list=list)